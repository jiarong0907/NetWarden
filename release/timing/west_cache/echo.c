...
int pre_ack_enabled;
int period; // the time interval to send pre acks
int static_pre_ack_period = DEFAULT_STATIC_PRE_ACK_PERIOD_MS;   // the period to scan the cache entry
int attack_start_from;
int defense_thres;
int loss = -1;
extern void print_stats_all_conns();
extern void check_cache_stuck(pcap_t *handle_out);
static long last_tstamp = 0;
double period_with_jitter; // the new period with jitter, default jitter is 0.
int base_rule_installed = 0;
int jitter_change_period = 1000; // the period to change jitters (ms)
extern double rtt_measured;
...
static void timer_handler(size_t timer_id, void *user_data)
{
   ...
   long tstamp = get_tstamp ();

   if (last_tstamp == 0)
   {
      last_tstamp = tstamp;
   } else if (tstamp - last_tstamp >= jitter_change_period){
      last_tstamp = tstamp;
      if (rtt_measured < 19 && rtt_measured > 12) {
         // period_with_jitter = 5;
         period_with_jitter = rtt_measured; // -15 to remove the delay of the cache
         printf("new period_with_jitter:%f\n", period_with_jitter);
      } else {
...
int main(int argc, char **argv)
{
   ...
   if (pre_ack_enabled) {
      if (period) {
         ...
         timer = start_timer(static_pre_ack_period, timer_handler,
                             TIMER_PERIODIC, (void *)handle_out);
