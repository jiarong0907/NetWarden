...
action generate_rand_rewinded_pkt() {
    subtract_from_field (tcp.ackNo, meta.pack_rand);
}

table generate_rand_rewinded_pkt_tab {
    actions {
        generate_rand_rewinded_pkt;
    }

    default_action  : generate_rand_rewinded_pkt;
    size : 1;
}

table generate_rand_rewinded_pkt_tab2 {
    actions {
        generate_rand_rewinded_pkt;
    }
...
/*===============================================================================================*/
/* generate 16 bits random number range [0, 15]*/

action get_rand () {
    modify_field_rng_uniform(meta.pack_rand_bit4, 0, 15);
}
...
table get_rand_tab{
    ...
    default_action  : get_rand;
    size : 1;
}


field_list pack_rand_list {
    meta.pack_rand_bit4;
}

field_list_calculation pack_rand_to_32 {
    input {
      pack_rand_list;
    }
    algorithm : identity_lsb;
...
action convert_pack_rand_to_32 () {
    modify_field_with_hash_based_offset(meta.pack_rand, 0, pack_rand_to_32, 4294967296);
}

table convert_pack_rand_to_32_tab {
    actions{
      convert_pack_rand_to_32;
    }
    default_action: convert_pack_rand_to_32;
    size: 1;
}
...
blackbox stateful_alu read_update_pkt_count_alu {
    reg : pkt_count;

    condition_lo: ig_intr_md.ingress_port == PORT_SERVER;
    condition_hi: tcp.flags != 18;

    update_lo_1_predicate: condition_lo and condition_hi;
    update_lo_1_value: register_lo + 1;

    output_predicate: true;
    output_dst: meta.pkt_count;
    output_value: register_lo;
}

action read_update_pkt_count() {
    read_update_pkt_count_alu.execute_stateful_alu(meta.conn_idx);
}

table read_update_pkt_count_tab {
    actions {
        read_update_pkt_count;
    }

    default_action : read_update_pkt_count;
    size : 1;
}
...
action sim_pack() {
    //subtract_from_field (tcp.ackNo, meta.covert_data);
    subtract_from_field (tcp.ackNo, 10);
...
table sim_pack_tab {
    ...
    default_action : sim_pack;
    size : 1;
}

action compute_pkt_count () {
    min(meta.min_pkt_count_512, meta.pkt_count, 512);
}

table compute_pkt_count_tab {

    actions {
        compute_pkt_count;
    }

    default_action  : compute_pkt_count;
    size : 1;
}

action compute_min_pkt_count_5 () {
    min(meta.min_pkt_count_5, meta.pkt_count, 5);
}

table compute_min_pkt_count_5_tab {

    actions {
        compute_min_pkt_count_5;
    }

    default_action  : compute_min_pkt_count_5;
    size : 1;
}
...
action cal_min_pack_11() {
    min(meta.min_flag_pack_11, meta.flag_pack, 11);
}

table cal_min_pack_11_tab {
    actions {
        cal_min_pack_11;
    }

    default_action : cal_min_pack_11;
    size : 1;
}
